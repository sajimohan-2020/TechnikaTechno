<script lang="ts">
  ...
  import Page from "../bookmarknav/BookMarkNav.svelte";
  ...
  // Match the actual CSS widths and gaps
  $: cardWidth = isMobile
    ? 300
    : innerWidth < 768
      ? 340
...
<section class="bg-[#F4F9FF] flex flex-col gap-5 p-4 lg:p-0 mt-20">
  <div class="flex flex-col lg:flex-row gap-5 px-4 xl:px-9">
    ...
    <div class="lg:pl-8 lg:pt-5 lg:w-[30%] xl:w-[35%] w-full px-4 lg:px-0 pt-8">
      {#each cardMain as cardDetails}
        ...
        <p
          class="text-gray-700 mb-8 lg:mb-12 xl:text-[18px] lg:text-[16px] md:text-[15px] text-[15px] text-left leading-tight"
        >
      ...
      <div class="hidden lg:flex justify-start items-center gap-10 mt-4 mb-4">
        <button
          class="bg-[#2D8AE7] text-white rounded-full h-16 w-16 p-2 hover:bg-blue-600 transition-colors flex items-center justify-center"
    ...
    >
      ...
      >
        {#each cardData as card}
          <div
            class="bg-white w-[300px] sm:w-[340px] md:w-[400px] lg:w-[480px] xl:w-[460px] mx-auto rounded-xl mb-4 p-2 sm:p-6 lg:p-[45px] xl:p-[55px] min-h-fit h-auto shrink-0 snap-start"
          >
